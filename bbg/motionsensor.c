...
void MotionSensor_init() {
    ...
    // Wrote in such a way it would be easy to swap GPIO pins ...
    // The GPIO49 has conflic with assn 4, so swap to GPIO48
    int len = snprintf(NULL, 0, GPIO, 48);
    char gpio_path[len];
    snprintf(gpio_path, len+1, GPIO, 48);

    // ...except this line
    // runCommand("config-pin p9.23 default"); // for some reason it only works at default?
    runCommand("config-pin p9.15 default");
    ...
    if (checkExported == NULL) {
        ...
        // fprintf(fExport, "%d", 49); // due to the conflict with as4, we use GPIO48 instead
        fprintf(fExport, "%d", 48);
        fclose(fExport);
    }
