...
static void *runner() {
    while (threadRunning) {
        ...
        // char *formatString = "ffmpeg -hide_banner -loglevel error -t %d -ar %d -f alsa -i %s -acodec mp3 -b:a 128k -f mp3 udp://%s:%d";
        char *formatString = "ffmpeg -hide_banner -loglevel error -ar %d -f alsa -i %s -acodec mp3 -f mp3 udp://%s:%d";

        // snprintf(buff, BUFFER_SIZE, formatString, DEFAULT_DURATION, RECORD_RATE, RECORDING_DEVICE_PCM, HOST, PORT);
        snprintf(buff, BUFFER_SIZE, formatString, RECORD_RATE, RECORDING_DEVICE_PCM, HOST, PORT);
        // printf(buff);
        runCommand(buff);
        // system(buff);
